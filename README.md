...
- **Shift + middle button → screen-space vertex move.** When an Editable Poly /
  Mesh is in **Vertex** sub-object level, **Shift + middle-drag** slides the
  selected verts along the screen plane. The middle button **alone always pans**
  as usual — only Shift+middle is intercepted, so there's no pan conflict (and
...
4. **Shift + middle button:** MAXScript keeps the hook's `VertexMoveArmed` flag
   ...
   **fully event-driven** via the `#selectionSetChanged` /
   ...
   hook intercepts the middle button only when armed *and* the modifier (Shift)
...
## Shift + middle drag in detail (screen-space vertex move)

Step by step, this is what a **Shift + middle-button** drag does:
...
2. **Middle button down.** In the low-level hook:
   - armed **and Shift held** (`GetAsyncKeyState`) → mark the drag, **swallow the
     event (return 1) so the viewport does not pan**, and flag a
     `VertexDragStart`.
   - otherwise → let it through, so the middle button **pans as normal**
     (middle alone always pans; Shift+middle outside vertex level is untouched).
...
```
Shift + middle down (vertex level) ──▶ pan suppressed
        │  drag
        ▼
